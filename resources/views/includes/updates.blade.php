							...
							<div class="card mb-4">
								...
								<div class="card-body pb-0">
									...
									<div class="media">
										...
										<div class="input-group promotext-div-update" data-value="{{$response->promotext}}">
										@if($response->promotext!=null && $response->promotext!='' && $response->locked=='yes')
									
										<div class="input-group-prepend"><span class="input-group-text">{{trans('general.write_promotext')}}</span></div><textarea class="form-control" id="promotext" name="promotext" value="{{$response->promotext}}" aria-label="With textarea">{{$response->promotext}}</textarea>
...
@if (auth()->check() && auth()->user()->id == $response->user()->id
	...
	@if(auth()->check() && $response->locked == 'no'|| auth()->check() && $response->locked=='yes' && $checkUserSubscription
	&& $response->price == 0.00||auth()->check() && auth()->user()->id == $response->user()->id || auth()->check() && $response->locked == 'yes'
	&& $checkUserSubscription
	&& $response->price != 0.00
	&& $checkPayPerView || auth()->check() && $response->locked == 'yes'
	&& $response->price != 0.00
	&& ! $checkUserSubscription
	&& $checkPayPerView)
...
@else
    <div class="card-body pt-0 pb-3">
		<p class="mb-0 update-text position-relative text-word-break">
			{!! Helper::linkText(Helper::checkText($response->promotext)) !!} 

			@if ($response->user()->price != 0.00 && $response->price == 0.00
				|| $response->user()->free_subscription == 'yes' && $response->price == 0.00)
			<a href="javascript:void(0);" @guest data-toggle="modal" data-target="#loginFormModal" @else @if ($response->user()->free_subscription == 'yes') data-toggle="modal" data-target="#subscriptionFreeForm" @else data-toggle="modal" data-target="#subscriptionForm" @endif @endguest class="link-dark">
				{{ trans('general.view_more') }}
			</a>
		@elseif ($response->user()->price != 0.00 && $response->price != 0.00
				|| $response->user()->free_subscription == 'yes' && $response->price != 0.00)
				<a href="javascript:void(0);" @guest data-toggle="modal" data-target="#loginFormModal" @else @if ($response->status == 'active') data-toggle="modal" data-target="#payPerViewForm" data-mediaid="{{$response->id}}" data-price="{{Helper::amountFormatDecimal($response->price, true)}}" data-subtotalprice="{{Helper::amountFormatDecimal($response->price)}}" data-pricegross="{{$response->price}}" @endif @endguest class="link-dark">
					@guest
						{{ trans('general.view_more') }}
					@else
