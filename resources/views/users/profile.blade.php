    ...
    <div class="row">
      <div class="col-lg-12">
        <div class="w-100 text-center py-4 img-profile-user">
          ...
          <div class="text-center position-relative @if ($user->isLive() && auth()->check() && auth()->user()->id != $user->id) avatar-wrap-live @else avatar-wrap @endif shadow @if (auth()->check() && auth()->user()->id != $user->id && Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes' || auth()->guest() && Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes') user-online-profile overflow-visible @elseif (auth()->check() && auth()->user()->id != $user->id && !Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes' || auth()->guest() && !Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes') user-offline-profile overflow-visible @endif">

            @if (auth()->check() && auth()->user()->id != $user->id && $user->isLive())
